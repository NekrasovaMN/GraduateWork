...
2. Поле "Месяц"
- двухзначное арабское число. С первого по двенадцатый месяц (01-12), но не ранее, чем дата заполнения, например: 12
3. Поле "Год"
- двухзначное арабское число, но не ранее, чем год заполнения и не позднее 5 лет от года заполнения, например: 23
4. Поле "Владелец"
...
*Ожидаемый результат:* Оплата проходит(данные об оплате появляются в таблице). Выходит сообщение "Успешно. Операция одобрена Банком."
...
*Ожидаемый результат:* Форма отправлена. Оплата проходит (данные об оплате появляются в таблице). Выходит сообщение "Успешно. Операция одобрена Банком."
...
*Ожидаемый результат:* Форма отправлена. Оплата не проходит (в базе данных появляется информация, что покупка отклонена). Выходит сообщение об ошибке "Ошибка!Банк отказал в проведении операции."
